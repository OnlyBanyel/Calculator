;(($) => {
  ...
  `)
      ...
      $(".fraction-bar").css("width", Math.max(Math.min(maxWidth * 0.8, 60), 30) + "px")
    ...
    // Also update the processFractionPart function to handle unclosed parentheses
    function processFractionPart(expression) {
      ...
      if (openParenCount > 0) {
        expression += ")".repeat(openParenCount)
      }

      // Handle square roots - convert √(x) to Math.sqrt(x)
      expression = expression.replace(/√$$([^()]+)$$/g, "Math.sqrt($1)")

      // Handle nested square roots
      while (expression.includes("√(") && expression.match(/√$$([^()]*$$[^()]+$$[^()]*)$$/)) {
        expression = expression.replace(/√$$([^()]*$$[^()]+$$[^()]*)$$/g, (match, p1) => "Math.sqrt(" + p1 + ")")
      }

      // Handle complex exponents with operations inside them
      expression = expression.replace(/(\d+\.?\d*|$$[^()]+$$)\^($$(?:[^()]*|\([^()]*$$)*\))/g, (match, base, exp) => {
        // Remove the outer parentheses from the exponent
        exp = exp.substring(1, exp.length - 1)
        return `Math.pow(${base}, ${exp})`
      })

      // Handle negative exponents and other operations in exponents without parentheses
      expression = expression.replace(/(\d+\.?\d*|$$[^()]+$$)\^(-?\d+\.?\d*)/g, "Math.pow($1, $2)")

      // Handle expressions like 2^√(4)
      expression = expression.replace(/(\d+\.?\d*|$$[^()]+$$)\^(Math\.sqrt$$[^()]+$$)/g, "Math.pow($1, $2)")

      // Handle any remaining exponents with a more careful approach
      let prevExpression
      do {
        prevExpression = expression
        // Look for patterns like "number^expression" and replace with Math.pow
        expression = expression.replace(/(\d+\.?\d*|$$[^()]+$$)\^([^+\-*/^()]+)/g, "Math.pow($1, $2)")
      } while (prevExpression !== expression)

      // Handle any remaining exponents that might involve operations
      expression = expression.replace(/(\d+\.?\d*|$$[^()]+$$)\^([^()]+)/g, (match, base, exp) => {
        // If the exponent contains operators, wrap it in parentheses
        if (/[+\-*/]/.test(exp)) {
          return `Math.pow(${base}, (${exp}))`
        }
        return `Math.pow(${base}, ${exp})`
      ...
      try {
        ...
        console.error("Evaluation error:", e, "Expression:", expression)
    ...
    // Evaluation function
    // Replace your evaluateWithPEMDAS function with this improved version:
    function evaluateWithPEMDAS(expression) {
      ...
      // Convert exponents to Math.pow format
      // Handle both ^(expression) and simple ^number cases
...
// Add this new helper function to properly handle exponents
function convertExponentsToMathPow(expression) {
  // First handle exponents with parentheses
  expression = expression.replace(/([\d.]+)\^\(([^)]+)\)/g, "Math.pow($1,$2)");
  
  // Then handle simple exponents without parentheses
  expression = expression.replace(/([\d.]+)\^([\d.]+)/g, "Math.pow($1,$2)");
  
  // Handle cases where the base is a parenthesized expression
  expression = expression.replace(/\(([^)]+)\)\^\(([^)]+)\)/g, "Math.pow($1,$2)");
  expression = expression.replace(/\(([^)]+)\)\^([\d.]+)/g, "Math.pow($1,$2)");
  
  // Handle exponents where the base is a sqrt
  expression = expression.replace(/(Math\.sqrt\([^)]+\))\^([\d.]+)/g, "Math.pow($1,$2)");
  
  return expression;
}

// Update your exponent button handler to ensure proper parentheses
// Update the exponent button click handler
// Update the exponent button handler
    ...
    // Exponent button
    // Update the exponent button click handler to better support complex expressions
    // Update the exponent button handler with proper duplicate prevention
$("#exponent").on("click", function(e) {
  ...
  // Check if we're already at a ^( sequence
  if (isFractionMode) {
    const pos = fractionCursorPos[activeFractionPart];
    const currentValue = fractionData[activeFractionPart];
    
    // Don't add if we're immediately after a ^
    if (pos > 0 && currentValue.substring(pos - 1, pos) === "^") {
      return;
    }
    
    // Don't add if we're at ^(
    if (pos > 1 && currentValue.substring(pos - 2, pos) === "^(") {
      return;
    }
  ...
  // For regular mode
  const beforeCursor = currentInput.substring(0, cursorIndex);
  
  // Don't add if we're immediately after a ^
  if (cursorIndex > 0 && currentInput.substring(cursorIndex - 1, cursorIndex) === "^") {
    return;
  }
  
  // Don't add if we're at ^(
  if (cursorIndex > 1 && currentInput.substring(cursorIndex - 2, cursorIndex) === "^(") {
    return;
  }
  ...
  // Insert ^( 
  currentInput = currentInput.slice(0, cursorIndex) + "^(" + currentInput.slice(cursorIndex);
  cursorIndex += 2;
  openParenCount++;
  updateDisplay();
});

    // Helper function to determine if we should add parentheses after the exponent symbol
    function shouldAddParentheses(value) {
      return true // For simplicity, always add parentheses for exponents
    }
    // Equals button
    $("#equals").on("click", () => {
      if (isRootMode) {
    ...
    function calculateRoot() {
      try {
        ...
        // Evaluate the radicand first in case it's an expression
        const radicandValue = evaluateWithPEMDAS(radicand);
        
        if (degreeNum === 0 || (degreeNum % 2 === 0 && radicandValue < 0)) {
          currentInput = "Error";
        } else {
          const result = Math.pow(radicandValue, 1 / degreeNum);
          currentInput = result.toString();
          history.push({
            equation: `${degreeNum}√(${radicand})`,
            result: currentInput,
          });
        }
      } catch (error) {
