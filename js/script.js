$(document).ready(() => {
  ...
  function calculateFraction() {
    ...
    try {
        ...
        const fractionStr = Number.isInteger(fractionResult) 
            ? fractionResult.toString() 
            : fractionResult.toFixed(10).replace(/\.?0+$/, "");

        // Combine with existing equation if there was one
        currentInput = currentEquation 
            ? `${currentEquation} ${fractionStr}`
            : fractionStr;

        history.push({
            equation: `${currentEquation || ""} ${fractionData.numerator}/${fractionData.denominator}`,
            result: currentInput,
        });
  ...
  function renderFractionWithCursor() {
    ...
    if (activeFractionPart === "numerator") {
        ...
        numeratorWithCursor = 
            ...
            '<span class="cursor-indicator"><span style="position: absolute; bottom: 0; width: 1px; height: 14px; background-color: black; animation: blink 1s infinite;"></span></span>' +
    ...
    // Show both the equation and the fraction
    $("#display").html(`
        <div style="display: flex; align-items: center; height: 100%;">
            <div style="margin-right: 10px;">${currentEquation || ""}</div>
            <div class="fraction-container">
                <div class="numerator">${numeratorWithCursor}</div>
                <div class="fraction-bar"></div>
                <div class="denominator">${denominatorWithCursor}</div>
            </div>
        </div>
    `);

    // Adjust the width of the fraction bar
    ...
    if (activeFractionPart === "numerator") {
        ...
        $numerator.scrollLeft(fractionCursorPos.numerator * 8 - $numerator.width() + 20);
    } else {
        const $denominator = $(".denominator");
        $denominator.scrollLeft(fractionCursorPos.denominator * 8 - $denominator.width() + 20);
    }
}
  ...
  function evaluateWithPEMDAS(expression) {
    ...
    expression = expression.replace(/\)(\d+)/g, ")*$1");    // )500 → )*500
    expression = expression.replace(/\)(\()/g, ")*$1");     // )( → )*(

    // Replace operators for JS
    expression = expression.replace(/×/g, "*").replace(/÷/g, "/");

    // First handle parentheses and exponents with proper precedence
    // This needs to be done carefully to maintain correct order
    
    try {
        // Create a parsing function that respects operator precedence
        const parseValue = (expr) => {
            // Handle parentheses first
            while (expr.includes("(")) {
                expr = expr.replace(/\(([^()]+)\)/g, (match, group) => {
                    return parseValue(group);
                });
            }
            
            // Then exponents (right associative)
            ...
            while (expr.match(/([\d.]+)[*/]([\d.]+)/)) {
                expr = expr.replace(/([\d.]+)([*/])([\d.]+)/g, (match, left, op, right) => {
                    return op === "*" 
  ...
  $("#fraction").on("click", () => {
    ...
    if (isFractionMode) {
        ...
        // Always start with empty fraction
        fractionData = {
            numerator: "",
            denominator: "",
        };
        ...
        // If there's existing input that's not just "0", keep it
        if (currentInput !== "0") {
